...
async function selectedProfilePage(req, res) {
  try {
    ...
    const profileUser = await userService.getPublicProfileByUsername(
      selectedUsername
    );
    if (!profileUser)
      return res
        .status(404)
        .render("errors/404", { message: "User not found" });
...
async function addFriend(req, res) {
  try {
    ...
    if (!targetId)
      return res
        .status(400)
        .json({ success: false, message: "Missing target id" });
...
const isFriend = async (req, res) => {
  ...
  try {
    const friends = await userService.getFriends(userId);
    res.json({isfriend: friends.includes(friendId)});
  } catch (err) {
    console.err();
    return res.status(500).json({ message: "internal server error" });
  }
